...
describe("Custom Tests", function () {
  ...
  it("[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances", async () => {
    ...
    // Alice withdraws a part of their funds from the shielded pool
    const aliceWithdrawAmount = utils.parseEther("0.08");
    const aliceChangeUtxo = new Utxo({
      amount: aliceDepositAmount.sub(aliceWithdrawAmount),
      keypair: aliceKeypair,
    });
    const aliceL2Address = "0xDeaD00000000000000000000000000000000BEEf";
    await transaction({
      tornadoPool,
      inputs: [aliceDepositUtxo],
      outputs: [aliceChangeUtxo],
      recipient: aliceL2Address,
    });

    const aliceBalance = await token.balanceOf(aliceL2Address);
    ...
    expect(omniBridgeBalance).to.be.equal(BigInt(0));
  });
  ...
  it("[assignment] iii. see assignment doc for details", async () => {
    ...
    // Alice withdraws remaining funds from the shielded pool
    ...
    const aliceAddress = "0x00000000000000000000000000000000ba5eba11";
    await transaction({
      tornadoPool,
    ...
    const poolBalance = await token.balanceOf(tornadoPool.address);
    expect(poolBalance).to.be.equal(BigInt(0));
    const omniBridgeBalance = await token.balanceOf(omniBridge.address);
